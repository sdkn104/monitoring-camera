...
Mat bgImg;
Mat fgSumm;
Mat detectMask; // mask_file
Mat detectMaskG; // mask_file gray
...
void help()
{
    ...
    << "or: ./bg_sub -vid 0    (for camera input)"                                  << endl
...
//     signal (SIGINT,sigint_handler);
    ...
    detectMask = imread(mask_file);
    cvtColor(detectMask, detectMaskG, CV_RGB2GRAY);
    threshold(detectMaskG, detectMaskG, 0, 255, THRESH_BINARY); // value=0 or 255
    event_id = 0; // init
    cnt_outputs = 0; // init
...
void handleFrame(int fno, const char *rootname) {
        ...
        Mat bin_img0;
        Mat bin_img;
        threshold(fgMask, bin_img0, 0, 255, THRESH_BINARY); // value=0 or 255
        bitwise_and(bin_img0,detectMaskG,bin_img); // mask by mask_file
